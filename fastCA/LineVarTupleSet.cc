...
{
  unsigned strength = varMapping[encode][0].size();
    for (int j = 0; j < strength ; ++j)
  {
    ...
    for (size_t i = 0; i < tuple.size(); ++i)
    {
      ...
      for (unsigned k = 0; k < strength ;k++)
        {
        if (varMapping[encode][i][j].row_index == varMapping[varTS.rbegin()->encode][varTS.rbegin()->column_index][k].row_index)
        {varMapping[varTS.rbegin()->encode][varTS.rbegin()->column_index][k].column_index = varMapping[encode][i][j].column_index;
            break;
        }
        }
        varTS.pop_back();
